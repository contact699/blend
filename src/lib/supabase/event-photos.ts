...
/**
 * Uploads an event photo to the private storage bucket
...
/**
 * Gets a signed URL for an event photo stored in the private bucket
 * Uses caching to avoid unnecessary API calls
 */
export async function getSignedEventPhotoUrl(
  storagePath: string,
...
/**
 * Deletes an event photo from storage
 */
export async function deleteEventPhoto(
  storagePath: string
): Promise<{ success: boolean; error: Error | null }> {
  try {
    const { error } = await supabase.storage.from('event-photos').remove([storagePath]);
...
/**
 * Complete event photo upload flow with validation
 */
export async function secureEventPhotoUpload(
  userId: string,
  imageUri: string
): Promise<{
  success: boolean;
  ...
  // Validate first
  const validation = await validateEventImage(imageUri);
  if (!validation.valid) {
    return { success: false, error: validation.error };
  }

  // Upload with EXIF stripping
  const { path, error } = await uploadEventPhoto(userId, imageUri);
